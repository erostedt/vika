...
static auto counting_input(vika::usize batch) -> vika::DeviceOwningTensorf
{
    ...
    return DeviceOwningTensorf::from(values, Extents::of(batch, 2, 2, 1)).unwrap();
}
...
UTEST(flatten, forward_collapses_every_dimension_but_the_batch)
{
    ...
    const auto layer = Flatten2DLayer::with_extents(Extents::of(2, 2, 2, 1)).unwrap();
...
UTEST(flatten, forward_smaller_batch)
{
    ...
    const auto inputs = counting_input(2);
    const auto layer = Flatten2DLayer::with_extents(Extents::of(4, 2, 2, 1)).unwrap();
...
UTEST(flatten, backward_smaller_batch)
{
    ...
    const auto upstream = DeviceOwningTensorf::from({1.0f, 2.0f, 3.0f, 4.0f}, Extents::of(1, 4)).unwrap();
    const auto layer = Flatten2DLayer::with_extents(Extents::of(4, 2, 2, 1)).unwrap();
...
UTEST(flatten, rejects_wrong_trailing_extents_in_both_directions)
{
    ...
    const auto wrong_input = DeviceOwningTensorf::zero(Extents::of(2, 3, 3, 1)).unwrap();
    EXPECT_TRUE(failed_with(layer.forward({wrong_input.const_view()}).wait(), ErrorKind::Shape));

    const auto wrong_upstream = DeviceOwningTensorf::zero(Extents::of(2, 5)).unwrap();
...
UTEST(flatten, with_extents_requires_rank_2_or_higher)
{
    ...
    EXPECT_TRUE(failed_with(Flatten2DLayer::with_extents(Extents::of(4)), ErrorKind::Shape));
    EXPECT_TRUE(Flatten2DLayer::with_extents(Extents::of(4, 2)).is_ok());
    EXPECT_TRUE(Flatten2DLayer::with_extents(Extents::of(4, 2, 2, 1)).is_ok());
}
