...
UTEST(concat, layer_forward)
{
    ...
    const auto b = DeviceOwningTensorf::from({10.0f, 20.0f, 30.0f, 40.0f, 50.0f, 60.0f}, Extents::of(2, 3)).unwrap();

    auto layer = ConcatLayer::with_extents({Extents::of(2, 2), Extents::of(2, 3)}).unwrap();
...
UTEST(concat, layer_forward_n_ary)
{
    ...
    const auto b = DeviceOwningTensorf::from({10.0f}, Extents::of(1, 1)).unwrap();
    const auto c = DeviceOwningTensorf::from({100.0f, 200.0f, 300.0f}, Extents::of(1, 3)).unwrap();

    auto layer = ConcatLayer::with_extents({Extents::of(1, 2), Extents::of(1, 1), Extents::of(1, 3)}).unwrap();
...
UTEST(concat, layer_forward_smaller_batch)
{
    ...
    const auto a = DeviceOwningTensorf::from({1.0f, 2.0f}, Extents::of(1, 2)).unwrap();
    const auto b = DeviceOwningTensorf::from({10.0f, 20.0f, 30.0f}, Extents::of(1, 3)).unwrap();
...
UTEST(concat, layer_forward_batch_exceeds_capacity)
{
    ...
    auto layer = ConcatLayer::with_extents({Extents::of(1, 2), Extents::of(1, 3)}).unwrap();
...
UTEST(concat, layer_forward_shape_mismatch)
{
    ...
    const auto wrong_b =
        DeviceOwningTensorf::from({1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f}, Extents::of(2, 4)).unwrap();
...
UTEST(concat, with_extents_rank_mismatch)
{
    ...
    ASSERT_TRUE(failed_with(ConcatLayer::with_extents({Extents::of(2, 2), Extents::of(2, 2, 2)}), ErrorKind::Shape));
}
...
UTEST(concat, with_extents_non_last_dim_mismatch)
{
    ...
    ASSERT_TRUE(failed_with(ConcatLayer::with_extents({Extents::of(2, 3, 4), Extents::of(2, 5, 4)}), ErrorKind::Shape));
}
...
UTEST(concat, layer_backward)
{
    using namespace vika;
    const auto upstream =
        DeviceOwningTensorf::from({1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f}, Extents::of(2,
            5)).unwrap();
